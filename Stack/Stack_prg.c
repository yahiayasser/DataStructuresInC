...
static Stack_ErrorType Stack_InsertAtFirst(Stack* STACK, Stack_DataSize data, Stack_node** head){
	Stack_node* Ptr_Elements = (Stack_node*)STACK;
	uint8 count = 0;
...
static Stack_ErrorType Stack_DeleteElement(Stack* STACK, Stack_node** head, uint8 index){
...
static Stack_ErrorType Stack_DeleteHead(Stack* STACK, Stack_node** head){
	if((*head) == NULL){
		return Stack_Empty;
	}
	else{
...
Stack_ErrorType Stack_Init(Stack* STACK, Stack_node** head){
	Stack_node* Ptr_Elements = (Stack_node*)STACK;
	uint8 count = 0;
	while(count != Stack_Max_Elements){
...
Stack_ErrorType Stack_Switch(Stack* STACK, Stack_node** head, uint8 Index1, uint8 Index2){
...
Stack_ErrorType Stack_UpdateElement(Stack* STACK, Stack_DataSize data, Stack_node** head, uint8 index){
	if(Stack_Check(STACK, head) == 0)
		return Stack_Notfound;
	Stack_node* current = (Stack_node*)*head;
	uint8 POS = 0;
...
Stack_ErrorType Stack_Push(Stack* STACK, Stack_DataSize data, Stack_node** head){
	if(Stack_Check(STACK, head) == 0)
		return Stack_Notfound;
...
#if(Stack_PRINT == Stack_PRINTING)
Stack_ErrorType Stack_Print(Stack* STACK, Stack_node** head){
	Stack_node* current = (Stack_node*)*head;
	if(current == NULL){
		return Stack_Empty;
	}
